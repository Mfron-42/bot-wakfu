...
namespace PacketEditor.WakfuBot.Bot.Actions
{
    public class DonjonsActions
    {
        ...
        public void LeavePepePaleSalle()
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToXFirst(new MapPosition(1, 0))));
            Manager.Send(TeleportAction.GetPacket(26240944509019648, 11));
        }

        public void EnterPepePaleSalle()
        {
            Manager.Send(TeleportAction.GetPacket(26247541578786048, 11));
        }
        ...
        public void LeaveSaleDjCrack(byte djLvl)
        {
            ...
            Manager.Send(PathMoveRequest.GetPacket(pathMove));
        ...
        public async Task LeaveDjGhostBonta()
        {
            ...
            {
                Manager.Send(new byte[] { 0x00, 0x11, 0x03, 0x00, 0xC9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x5F, 0xB3, 0x00, 0x05, 0x00, 0x00 });//utilise la sortie
                await Task.Delay(50);
            }
        }
        ...
        public void LeaveDjTsar(byte djLvl)
        {
            ...
            Manager.Send(PathMoveRequest.GetPacket(exit));
            Manager.Send(new byte[] { 0x00, 0x16, 0x03, 0x00, 0xCD, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x17, 0x67, 0x00, 0x00, 0x00, 0x87, 0x00, 0x00, 0x00, 0x00, 0x0B });
            Manager.AddOneExecutionAction(PacketType.InteractiveElementSpawn, (InteractiveElementSpawn o)
                => Manager.Send(new byte[] { 0x00, 0x16, 0x03, 0x00, 0xCD, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0xEE, 0x00, 0x00, 0x02, 0xE3, 0x00, 0x00, 0x00, 0x00, djLvl }));//ghosto
        }
        ...
        public void LeaveFinDjChat()
        {
            ...
            Manager.Send(TeleportAction.GetPacket(15958311765721088, 11));
        }

        public void LeaveFirstSalleChat()
        {
            var exit = BotActions.MainPlayer().GetPosition().GoToYFirst(new MapPosition(0,0));
            Manager.Send(PathMoveRequest.GetPacket(exit));
            Manager.Send(TeleportAction.GetPacket(15957212254093312, 11));
        }
        
        public void EntrerDjChat(int djLvl)
            => Manager.Send(TeleportAction.GetPacket(20600449858193664, djLvl));
        ...
        public void LeaveDjPerlouze(int djLvl)
        {
            ...
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToYFirst(bossStep1)));
            Manager.Send(new byte[] { 0x00, 0x1C, 0x03, 0x10, 0x11, 0xFF, 0xFF, 0xFF, 0xC5, 0xFF, 0xFF, 0xFF, 0xE4, 0xFF, 0xF4, 0x0C, 0xDF, 0xA0, 0xA0, 0xBF, 0xE0, 0xE0, 0xE0, 0xE0, 0xE2, 0xE3, 0xE0, 0xA0 });
        ...
        public void LeaveGeleeFirstSalle(int djLvl)
        {
            ...
            Manager.Send(new byte[] { 0x00, 0x19, 0x03, 0x10, 0x11, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x09, 0x60, 0x62, 0x62, 0x60, 0x7E, 0x7E, 0x60, 0x60, 0x80 });
            Manager.Send(TeleportAction.GetPacket(21071040835015680, 11));

            Manager.Send(new byte[] { 0x00, 0x11, 0x03, 0x10, 0x11, 0xFF, 0xFF, 0xFF, 0xF3, 0xFF, 0xFF, 0xFF, 0xCE, 0x00, 0x14, 0x01, 0xE0 });
        ...
        public void LeaveSallePerlouze(int djLvl)
        {
            ...
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToYFirst(moveExit)));
            Manager.Send(TeleportAction.GetPacket(18667508416628992, 11));
        }

        public void LeaveBlackWabbit(int djLvl)
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToXFirst(new MapPosition(-2, -15, -7))));
            Manager.Send(new byte[] { 0x00, 0x20, 0x03, 0x10, 0x11, 0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFF, 0xFF, 0xF1, 0xFF, 0xF9, 0x10, 0x40, 0x40, 0x60, 0x60, 0x60, 0x60, 0x61, 0x61, 0x61, 0x60, 0x61, 0x61, 0x60, 0x61, 0x61, 0x61 });
        ...
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToYFirst(new MapPosition(0, -5))));
            Manager.Send(PathMoveRequest.GetPacket(new MapPosition(0, -5).GoToXFirst(new MapPosition(0, 1))));
        ...
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToYFirst(new MapPosition())));
        ...
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToXFirst(new MapPosition(0, 1))));
        ...
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToYFirst(new MapPosition(1, 0))));
        ...
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToXFirst(new MapPosition(0, -8))));
        ...
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToYFirst(new MapPosition(7, 11))));
            Manager.Send(PathMoveRequest.GetPacket(new MapPosition(7, 11).GoToYFirst(new MapPosition(7, 19))));
        ...
        {
            Manager.Send(PathMoveRequest.GetPacket(BotActions.MainPlayer().GetPosition().GoToXFirst(new MapPosition(208, 97))));
