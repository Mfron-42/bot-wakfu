...
{
    public class SalleDjSrambad : ABotActionConfig
    {
        public static new string Name { get; set; } = "Salle Srambad";

        public SalleDjSrambad(WakfuDatas manager, Map map, Dictionary<long, APlayerService> players, DonjonsActions donjonsActions, int djLvl)
        ...
        {
            DonjonsActions.EnterDjSrambad(DjLvl);
            base.Start();
        }
        ...
        public override void AddConfig()
        {
            ...
            AddConstantAction(PacketType.EndFightCreation, (EndFightCreation o) =>
            {
                if (Players.Count > 1)
                {
                    Send(FightPlacement.GetPacket(new MapPosition(-5, -1, 0), MainPlayer().PlayerId));
                    Send(FightPlacement.GetPacket(new MapPosition(-5, 2, 0), Players.ElementAt(1).Value.PlayerId));
                }
                if (Players.Count > 2)
                    Send(FightPlacement.GetPacket(new MapPosition(-5, -4, 0), Players.ElementAt(2).Value.PlayerId));
