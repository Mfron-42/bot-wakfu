...
namespace WakfuBot.WakfuBot.Bot.Actions.ActionsConfig
{
    public class SalleDjKaniboule
    {
        ...
        public SalleDjKaniboule(WakfuDatas manager, Map map, Dictionary<long, APlayerService> Players, DonjonsActions donjonsActions)
        {
            this.Map = map;
        ...
        public void Debug()
        {
            Manager.Send(TeleportAction.GetPacket(28112313299557632, 1));
        }
        ...
        private void Config()
        {
            ...
            Manager.AddConstantAction(PacketType.FightEnd, (FightEnd o) =>
            {
                if (o.FightId != Map.FightId)
                    return;
                DonjonsActions.LeaveSalleKaniboule(1);
            });
            ...
            {
                Manager.Send(FightPlacement.GetPacket(new MapPosition(5, -10), MainPlayer().PlayerId));
